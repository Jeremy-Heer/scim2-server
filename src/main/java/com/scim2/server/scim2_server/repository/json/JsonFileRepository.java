package com.scim2.server.scim2_server.repository.json;
...
import com.scim2.server.scim2_server.service.SdkFilterService;
...
@Repository
@ConditionalOnProperty(name = "scim.backend.type", havingValue = "json", matchIfMissing = true)
public class JsonFileRepository implements ScimRepository {
    ...
    public JsonFileRepository(SdkFilterService sdkFilterService, ScimAttributeService scimAttributeService, 
    ...
    public List<UserResource> searchUsers(String filter, String attributes, String excludedAttributes, 
        ...
        // Apply pagination
        int start = (startIndex > 0) ? startIndex - 1 : 0;
        int end = (count > 0) ? Math.min(start + count, allUsers.size()) : allUsers.size();
    ...
    public List<GroupResource> searchGroups(String filter, String attributes, String excludedAttributes, 
        ...
        // Apply pagination
        int start = (startIndex > 0) ? startIndex - 1 : 0;
        int end = (count > 0) ? Math.min(start + count, allGroups.size()) : allGroups.size();
